...
1. **High Efferent/Afferent Coupling (The Hubs):** Modules acting as the API entry points or core orchestrators (`src/main/core.js`, `src/main/index.js`) exhibit high Afferent Coupling (Fan-in). They act as architectural gravity centers, absorbing complexity to provide a unified API to the CLI and external plugins. If these modules break, the entire system collapses.
